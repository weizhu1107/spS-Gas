...
for i in `seq 1 $num_reg`;do
  ...
  $BIN_DIR/script/init_legend_dir.py $f $LOW_BOUND $UP_BOUND $LGD_DIR/$i
done
...
for i in `seq 1 $num_reg`;do
	if [ -f $LGD_DIR"/"$i".txt" ];then
		echo $i
		Rscript $BIN_DIR/script/causal_allel.R $LGD_DIR"/"$i".txt" $BASIS_DIR"/causal_list/"$i".txt" $prev $nc
	fi
done
